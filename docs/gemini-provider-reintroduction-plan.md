...
**What the "state" is (binary-level detail).** From the shipped Linux x64
`language_server` (stripped ELF; symbols recovered from the Go pclntab), the
method is `google3/third_party/jetski/language_server/language_server.(*Server).getProxyServerURLWithState`
- state on the server object, populated at process startup. Two proxy inputs
exist in the binary, and the IDE sets **neither**:

1. **A `proxy_server_url` startup flag.** It sits in the binary's flag cluster
   alongside the launcher flags the IDE does use (`http_server_port`,
   `parent_pipe_path`, `use_local_chrome`, `browser_eval_env`). The argv built
   in `extensions/antigravity/dist/extension.js` is fixed - `--lsp_port`,
   `--https_server_port`, `--parent_pipe_path`, `--cloud_code_endpoint`,
   `--subclient_type ide`, `--app_data_dir`, `--workspace_id`,
   `--persistent_mode`, `--use_local_chrome`, ... - and never includes
   `--proxy_server_url`, so the state's proxy URL is always empty.
2. **`UserSettings.detect_and_use_proxy`** (`codeium_common_pb`, enum
   `UNSPECIFIED`/`DISABLED`/`ENABLED`, generated `GetDetectAndUseProxy`
   accessor - a Codeium/Windsurf lineage leftover). Zero references in the
   IDE's JavaScript, so it is never sent either.

The IDE's only proxy plumbing is reading VS Code's `http.proxy` setting and
exporting `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` into the child env - exactly
the environment the explicit `ProxyURL` overrides.

Implications: (a) the upstream ask can be sharpened - besides "fall back to
`ProxyFromEnvironment`", Antigravity could simply pass/expose the
`--proxy_server_url` flag (or wire `detect_and_use_proxy`) that already ships
in the binary; (b) a local, update-hostile test exists: patch
`dist/extension.js` to append `--proxy_server_url http://127.0.0.1:<engine port>`
to that argv (Go uses the system trust store, so the Gate CA should pass TLS).
Useful to confirm the flag routes the model client before filing upstream; not
something Gate Connect can ship.
