...
/// visible on any backdrop. A colored status dot is composited on top: for a
/// dead OAuth session (`needs_signin`) a red "sign in required" dot is drawn on
/// **every** platform, since that exceptional state warrants an at-a-glance cue
/// even on Windows/Linux; otherwise the routine routing dot (green when the
/// proxy is routing, gray when off) is drawn on **macOS only**, leaving the
/// Windows/Linux glyph as the tinted mark alone as before.
#[cfg(any(target_os = "macos", target_os = "windows", target_os = "linux"))]
fn tray_image(proxy_on: bool, needs_signin: bool, dark_menubar: bool) -> Option<Image<'static>> {
    ...
    // Pick the status-dot color, or skip the dot entirely. Attention (dead
    // session) draws a red dot on every platform; the routine routing dot stays
    // macOS-only so Windows/Linux keep their bare tinted mark when signed in.
    let dot: Option<(u8, u8, u8)> = if needs_signin {
        Some((0xE5, 0x48, 0x4D)) // red - sign in required
    } else {
        #[cfg(target_os = "macos")]
        {
            Some(if proxy_on {
    ...
    // Composite the dot, bottom-right.
    if let Some((dr, dg, db)) = dot {
...
/// light vs dark menu bar / taskbar, and overlay the status dot (macOS routing
/// dot, or the red sign-in-required dot on any platform - see `tray_image`).
/// Also refreshes the tooltip on macOS + Windows.
#[cfg(any(target_os = "macos", target_os = "windows", target_os = "linux"))]
