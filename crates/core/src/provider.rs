...
pub fn providers() -> Vec<Provider> {
    vec![
        ...
        Provider {
            ...
            // The Gemini CLI (Node) only reads HTTP(S)_PROXY env vars, so the
            // proxy reaches it via env injection: Linux through the
            // environment.d drop-in, macOS through the managed ~/.zshenv
