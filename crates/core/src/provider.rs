...
/// requests straight to the provider once the engine is gone - except a tool on
/// Gate pay-as-you-go, whose config carries no provider credential to go direct
/// with, and which the forwarder answers with an error saying to open Gate
/// Connect instead. It is left as it is too, because routing resumes on the
/// next start with nothing rewritten. A config naming
