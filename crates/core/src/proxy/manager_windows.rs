...
impl ProxyManager {
    ...
    pub fn enable(&self) -> Result<ProxyState> {
        ...
        // that go through WinINET; the CLI AI tools read `HTTPS_PROXY` instead,
        // and OpenCode has no proxy setting of its own at all. Deliberately
        // best-effort: a registry failure must not take routing down for
        // everything that *does* follow the PAC, so it degrades to "GUI apps
        // routed, CLI tools not" rather than to "enable failed".
