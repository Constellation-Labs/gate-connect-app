...
///
/// The catalog is environment-independent. The chat surfaces (`claude-web`,
/// `chatgpt-apps`) were gated to the staging gateway while the gateway-side
/// classification they depend on (`CLAUDE_WEB_CHAT_COMPLETION_RE` and the
/// ChatGPT anchors beside it) was staging-only; that classification is
/// deployed to production, so the gate is gone. They remain opt-in through
/// their catalog `enabled: false`, which is a different thing from `supported`
/// and the only thing keeping a session-cookie surface off by default.
