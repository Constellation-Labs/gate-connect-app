...
//! why the tool segment is worth a path segment. The credential lives in the keychain and is
...
fn inject_credential(
    ...
    // news here.
    // The relay's half of the established-tool pair. There is no route selector
    // here and there cannot be one - a reverse proxy a tool points its base URL
    // at sees no CONNECT and no `Proxy-Authorization` to read - but that base
    // URL is ours to write, so the tool rides in its path instead
    // ([`TOOL_PATH_PREFIX`]). Same fact and same precedence as the engine's
    // selector, established by the only means this transport has.
...
fn split_tool_segment(path_and_query: &str) -> (Option<&'static str>, String) {
    let Some(rest) = path_and_query.strip_prefix(TOOL_PATH_PREFIX) else {
        return (None, path_and_query.to_string());
    };
    ...
    (
        crate::registry::ToolId::from_slug(segment).map(crate::registry::ToolId::slug),
        inner,
    )
}
...
fn resolve_route(
    ...
    // and everything below sees the path it always saw.
