 ...
 * `Components / Sidenav`, the `logo` component set, read 2026-08-23).
 *
 * Stopgap geometry: the paths are vendored from lobehub/lobe-icons
 * (https://github.com/lobehub/lobe-icons, MIT) until the designer exports the
 * drawn marks - both are the same official logos, so when that lands only this
 * file changes. The marks remain their owners' trademarks, used here to
 * identify the tools they name.
 *
 * Everything fills `currentColor`, so the dark tile's white text colours the
 * mark and nothing here picks a colour of its own.
 */
...
const PATHS: Record<BrandName, JSX.Element> = {
  ...
  // The source wraps these in a clip to the 24px viewBox and carries five
  // unreferenced gradients; both dropped - the svg element clips to its
  // viewBox on its own.
  openclaw: (
    <>
...
/**
 * Which mark a tool or proxy-domain slug carries. One map for both namespaces:
 * they never collide (the `opencode` tool and domain are the same brand), and
 * the rail mixes the two kinds of row anyway.
 */
const BRAND_BY_SLUG: Record<string, BrandName> = {
