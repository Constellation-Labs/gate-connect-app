...
const SKIN: Record<PillLabel, { wrap: string; dot: string }> = {
  ...
  // the same argument that gives "No routing" ink-2 gives this the
...
export function groupPillLabel(
  ...
  if (group.members.length === 0 || group.routed === 0) {
    ...
    //
    // `not-routing` cannot. It is set as `enabled && !proxyOn`, and `proxyOn` is
...
export function memberPillLabel(member: GroupMember): PillLabel {
  ...
  // didn't turn it off).
  if (member.attention === "not-routing") return "No routing";
